...
calc_nwstats_msm <- function(time.unit,
  ...
  if (!is.na(qnts.B[1]) & !is.na(qnts.W[1])) {
      ...
      if (top5==TRUE) {
        ...
        num.riskg <- c(0.4,0.3,0.15,0.1,0.04,0.01) * num * qnts.B * time.unit
        } 
...
base_nw_msm <- function(nwstats) {
  ...
  if (nwstats$top5==TRUE){   
    ...
    riskg.B <- sample(apportion_lr(num.B, 1:6, c(0.4,0.3,0.15,0.1,0.04,0.01)))
    riskg.W <- sample(apportion_lr(num.W, 1:6, c(0.4,0.3,0.15,0.1,0.04,0.01)))
  } else {
